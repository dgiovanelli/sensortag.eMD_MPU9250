...
char UartIn;
static UartReceivedCb_t UartReceivedCb = NULL;
...
bool ini_uart(void){
    ...
    uartParams.readReturnMode = UART_RETURN_FULL;// UART_RETURN_NEWLINE;//UART_RETURN_FULL;
    uartParams.readEcho = UART_ECHO_OFF;//UART_ECHO_OFF;
    uartParams.baudRate = 115200;						//TODO: Why does it only works for 9600?? Because im debuggin? With others bauds it bricks
...
//Trigers uart read and catches the callback
void uartRead(void)
{
    UART_read(uart, &UartIn, 1);
}

//Register the callback when uart has been received
bool reg_int_UART_cb(UartReceivedCb_t pfn){
	UartReceivedCb = pfn;
	return true;
}

char uartRetRecv(void){
	return UartIn;
}
