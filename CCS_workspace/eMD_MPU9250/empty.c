...
Void dummyTask(UArg arg0, UArg arg1) {
	...
	MPL_LOGI("BOOTING:\n")
;
	//COMMUNICATION PERIPHERAL SETUP
	ledPinHandle = PIN_open(&ledPinState, ledPinTable);
	if (!ledPinHandle) {
		MPL_LOGE("Error pines\n");
	} else {
		MPL_LOGI("PINs initialized!\n")
;	}

	if (!SensorI2C_open()) {
		MPL_LOGE("Error de i2c\n");
	} else {
		MPL_LOGI("I2C initialized!\n")
;	}
...
;	}
	...
	//PUT SENSORS IN LOW POWER
	SensorBmp280_init();
	SensorBmp280_enable(FALSE);
	SensorHdc1000_init();
	//SensorMpu9250_powerOff(); //used and managed by the application through the MPL library
	SensorOpt3001_init();
	SensorTmp007_init();
	delay_ms(100);
...
;	}
	...
	PIN_setOutputValue(ledPinHandle, Board_LED0,!PIN_getOutputValue(Board_LED0));
		...
		if (new_uart) {
			new_uart = FALSE;
			uartRead();
			handle_input();																	//Pide caracter via uart, sin bloquear
		}
		...
		if (hal.motion_int_mode) {
			...
			counter++;
			if(counter >= 2){
				mpu_lp_motion_interrupt(0, 0, 0);/* Restore the previous sensor configuration. */
				counter = 0;
...
#endif
//		if (hal.report & PRINT_PEDO) { //if only pedometer is enabled something has to trigger the reading
//			read_from_mpl();
//		}
		if (new_data) {
			inv_execute_on_data();
...
int main(void) {
	...
	Task_construct(&task0Struct, (Task_FuncPtr) dummyTask, &taskParams, NULL);
